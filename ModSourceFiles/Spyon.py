...
class Spyon:
	...
	def OnServerCommand(self, arg):
		if not (arg.cmd and arg.cmd.FullName and self.is_rcon_or_allowed(arg)):
			return
		p = self.name(arg)
		cmd = arg.cmd.FullName
		args = arg.FullString
	...
	def reason_con_hook(self, arg):
		if not self.is_rcon_or_allowed(arg):
			return
		self.log_reason(self.name(arg), arg.FullString)
		print("Reason logged.")
