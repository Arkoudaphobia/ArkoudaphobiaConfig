...
{
    [Info("Jail", "Reneb / k1lly0u", "3.0.13", ResourceId = 1649)]
                        ...
                        if (success is bool)
                        {
                            if ((bool)success)
                            {
                                ...
                                if (args.Length >= 4 && broadcastImprisonment)
                                    PrintToChat(string.Format(lang.GetMessage("sentToReason", this, player.UserIDString), target.displayName, args[4]));
                            }                          
                        }
